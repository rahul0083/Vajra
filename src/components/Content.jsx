import React from 'react'
import Login from './Login'
import TouchAppIcon from '@material-ui/icons/TouchApp';
import TrackChangesIcon from '@material-ui/icons/TrackChanges';
import MonetizationOnIcon from '@material-ui/icons/MonetizationOn';


function Content() {
  return (
    <>
        <div className='content'>
     
      <div className='loginpage_left'>
      <h2 className='features'>Features</h2>
       <div className='icons'>
       <div> <TouchAppIcon /> </div>
        <div style={{fontSize:"22px",marginLeft:"10px"}}>   Digitize
       <p className='description'>A Bird eye view of the entire Biomedical Operations. </p>
       </div>
       </div>
       <div className='icons'>
       <div> <TrackChangesIcon />  </div>  
       <div style={{fontSize:"22px",marginLeft:"10px"}}>  Track
        <p className='description'>Track the product life cycle and service history.</p>
       </div>
       </div>
       <div className='icons'>
      <div> <MonetizationOnIcon />  </div>  
      <div style={{fontSize:"22px",marginLeft:"10px"}}>  Biomedical Budget
        <p className='description'>Regulate and save budget through analysed data and informed</p>
       </div>
       </div>
      </div>
      <div className='loginscreen'>
        <Login />
      </div>
      
    </div>
 
   <p style={{display:"flex",justifyContent:"center" ,textAlign:"center"}}> 
   <p style={{color:" #004F95"}}>  Download our app </p> 
   <div> <a href="/"> <img style={{height:"56px"}} src='https://play.google.com/intl/en_us/badges/static/images/badges/en_badge_web_generic.png' />
   </a> </div>
    <div> <a href=""> <img style={{height:"38px",marginTop:"9px"}} src="data:image/png;base64," alt="" />
   </a> </div>
    </p>   
    </>
  )
}

export default Content
